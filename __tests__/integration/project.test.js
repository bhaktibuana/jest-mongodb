...
describe("Project", () => {
  ...
  it("should be able to update a project", async () => {
    ...
    const responseGet = await request(app).get(
      `/projects/${responseUpdate.body._id}`
    );
  ...
  it("should show sorted projects", async () => {
    ...
    await request(app).post("/projects").send(data[0]);
    await request(app).post("/projects").send(data[1]);
    await request(app).post("/projects").send(data[2]);

    const response = await request(app).get("/projects-sorted");
